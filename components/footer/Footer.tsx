  ...
  return (
    <footer className="relative bg-gradient-to-br from-gray-50 via-[#FFF8E7] to-gray-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 border-t border-gray-200 dark:border-gray-800 overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none">
        <h1 className="text-[20vw] md:text-[25vw] font-black text-gray-900/[0.01] dark:text-white/[0.02] leading-none tracking-tighter">
          HENKO
        </h1>
      </div>
        ...
        <div className="text-center mb-12">
          <Link href="/" className="inline-flex items-center gap-3 group">
            <div className="flex items-center">
              ...
              <span className="text-gray-800 dark:text-white text-2xl font-bold ml-2">Henko Group <span className="text-primary">.</span></span>
              </div>
          </Link>
        </div>
          ...
          <ul className="flex flex-wrap items-center justify-center gap-x-8 gap-y-4">
            {footerLinks.map((link) => (
              <li key={link.name}>
                <Link
                  href={link.href}
                  className="text-gray-600 dark:text-gray-400 hover:text-primary-content dark:hover:text-white font-medium transition-colors duration-200 text-sm"
                >
                  {link.name}
                </Link>
              </li>
